...
func TestMakeTreeSuccess(t *testing.T) {
	for _, tt := range successTestCases {
		...
		if err != nil {
			var _ error = err
			t.Errorf("Build for test case %q returned error %q. Error not expected.",
				tt.name, err)
		}
		if !reflect.DeepEqual(actual, tt.expected) {
			...
			t.Errorf("Build for test case %q returned %s but was expected to return %s.",
...
func TestMakeTreeFailure(t *testing.T) {
	for _, tt := range failureTestCases {
		actual, err := Build(tt.input)
		if err == nil {
			t.Errorf("Build for test case %q returned %s but was expected to fail.",
				tt.name, actual)
		}
	}
}
